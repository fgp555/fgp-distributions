...
class FeedbackController {
    ...
    static async getOne(req, res) {
        const feedback = await feedback_service_1.FeedbackService.getById(Number(req.params.id));
        if (!feedback) {
    ...
    static async delete(req, res) {
        const success = await feedback_service_1.FeedbackService.delete(Number(req.params.id));
